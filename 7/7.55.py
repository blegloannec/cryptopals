...
def modification(M):
    ...
    # resulting block should work with proba. 2^(-17) ~ 2^(-21)
    ...
    # as we re-modify m0, we have to refresh a1
    # this won't interfere with the condition on a1 though
    A[1] = lrot((A[0] + F(B[0],C[0],D[0]) + M[0]) & msk32, 3)
    # since a1 has been modified, this changes d1, c1, b1 & a2,
    # we then have to update m1 to m4 to get back the proper
    # values of d1 to a2
